...
class _AppComplaintState extends State<AppComplaint> {
  ...
  void fetchComplaints() async {
    ...
    final Uri apiUrl = Uri.parse(url+'view_complaint_reply');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'ok') {
          setState(() {
            complaints = List<Map<String, dynamic>>.from(data['data'] ?? []);
            isLoading = false;
          });
        } else {
      ...
      setState(() {
        isLoading = false; // Ensure loading state is updated in all cases
      });
    }
  }
  ...
  void sendComplaint() async {
    ...
    String url = sh.getString('url') ?? '';
    String lid = sh.getString("lid").toString();
    final Uri apiUrl = Uri.parse(url+'send_complaint');
  ...
  void deleteComplaint(String complaintId) async {
    SharedPreferences sh = await SharedPreferences.getInstance();
    String url = sh.getString('url') ?? '';
    final Uri apiUrl = Uri.parse('$url/delete_complaint');

    try {
      final response = await http.post(apiUrl, body: {'complaint_id': complaintId});
